...
CREATE DATABASE conduit WITH OWNER = cond ENCODING = 'UTF8' LC_COLLATE = 'C' LC_CTYPE = 'C' TABLESPACE = pg_default CONNECTION
LIMIT = - 1;

COMMENT ON DATABASE conduit IS 'Database for the Real World Haskell blogging application.';

-- For no, do not secure access = one user has all privileges.
...
GRANT ALL PRIVILEGES ON DATABASE conduit TO conduit_all GRANT conduit_all TO cond;
