...
unit_validTag :: IO ()
unit_validTag =
  HU.assertBool "Can a valid tag be constructed?" (isJust $ mkTag "testTag")

unit_tagTooShort :: IO ()
unit_tagTooShort = HU.assertBool
  "A tag that is less than 3 characters cannot be constructed"
  (isNothing $ mkTag "tg")

unit_tagTooLong :: IO ()
unit_tagTooLong = HU.assertBool
  "A tag that is longer than 20 characters cannot be constructed"
  (isNothing $ mkTag "ThisTagIsWayTooLongToBeValid")

unit_noControlCharInTag :: IO ()
unit_noControlCharInTag = HU.assertBool
  "A tag cannot contain control characters"
  (isNothing $ mkTag "Tag\^QWith\^PControl")
