...
unit_validTitle :: IO ()
unit_validTitle = HU.assertBool "Can a valid Title be constructed?" (isJust $ mkTitle "The Title of my Blog Post")

unit_canonicalTitle :: IO ()
unit_canonicalTitle = HU.assertEqual "Is a title string converted to canonical form?" (getTitle . fromJust . mkTitle $ " A  not Canonical   Title   ") "A not Canonical Title"
...
prop_validation (TU.Utf8Text1 t) = case mkTitle t of
  ...
  where
    isValidTitle tt =
      not (T.null tt)
        && T.all (\x -> TL1.isAlphaNum x || TL1.isWhiteSpace x) tt
